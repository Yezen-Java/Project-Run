...
$getLocationMedia = pg_query("SELECT * From location_res, media where location_res.locationid = '{$escapeGetLocationId}' and location_res.mediaid = media.mediaid;");
	...
	if($getLocationMedia){
        ...
        $getContent = $getContent."<dt class ='$mediaId' value ='$mediaId'><button class='glyphicon glyphicon-trash' id='trashBoxMedia' value ='$mediaId' onclick='deleteMediaLi(this.value);'></button> <a>$media_name</a> </dt>";
	}
}else{
	return $getContent;
}
