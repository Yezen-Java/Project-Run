...
$getLocationMedia = pg_query("SELECT * From location_res, media where location_res.locationid = '{$escapeGetLocationId}' and location_res.username = '{$username}' and location_res.mediaid = media.mediaid;");
	...
	if($getLocationMedia){
        ...
        $getContent = $getContent."<li class ='tourLoactions'><button class='glyphicon glyphicon-trash' id='trashBoxMedia' value ='$mediaId' onclick='deleteMediaLi(this.value);'></button><div class ='$mediaId' value='$mediaId'></div> <a>$media_name</a> </li>";
	}
}else{
	return $getContent;
}
